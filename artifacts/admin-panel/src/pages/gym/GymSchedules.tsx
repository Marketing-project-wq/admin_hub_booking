...
// GYM — Kelola Jadwal. Mirror kemampuan ArenaSchedules, TAPI hanya baca/tulis
// tabel gym_* (gym_class_schedules, gym_class_types, gym_coaches, gym_class_bookings).
// Skema gym lebih ramping: tanpa unit_id / notes / cancelled_reason / cutoff_minutes /
// updated_at pada gym_class_schedules; durasi kelas di kolom duration_minutes.

interface Schedule {
  id: string; class_type_id: string; schedule_date: string;
  start_time: string; end_time: string; instructor: string; quota: number;
  is_cancelled: boolean; created_at: string;
  class_type?: { id: string; name: string; color: string; duration_minutes: number };
  booked_count?: number;
}
interface ClassType { id: string; name: string; color: string; duration_minutes: number }
...
export default function GymSchedules() {
  ...
  useEffect(() => {
    Promise.all([
      supabase.from('gym_class_types').select('id, name, color, duration_minutes').eq('is_active', true).order('name'),
  ...
  const fetchData = useCallback(async () => {
    ...
    let q = supabase
      ...
      .select(`
        id, class_type_id, schedule_date, start_time, end_time,
        instructor, quota, is_cancelled, created_at,
        class_type:gym_class_types(id, name, color, duration_minutes)
      `)
  ...
  const autoEndTime = (start: string, ctId: string) => {
    ...
    const total = h * 60 + m + ct.duration_minutes
  ...
  const openEdit = (s: Schedule) => {
    setForm({
      ...
      end_time: s.end_time.slice(0, 5), quota: s.quota,
    })
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const { error: err } = editId
      ? await supabase.from('gym_class_schedules').update(base).eq('id', editId)
      : await supabase.from('gym_class_schedules').insert({ ...base, is_cancelled: false })
    setSaving(false)
  ...
  const openCancelModal = async (s: Schedule) => {
    setConfirmCancel(s); setCancelError('')
  ...
  const closeCancelModal = () => {
    setConfirmCancel(null); setCancelError('')
    setAckCancel(false); setMoveMode(false); setMoveTargetId(null); setMoveTargets([])
    setCancelConfirmedCount(null)
  }
  ...
  const fetchMoveTargets = async (current: Schedule) => {
    ...
    const { data: rows } = await supabase
      .from('gym_class_schedules')
      .select('id, schedule_date, start_time, end_time, instructor, quota, class_type:gym_class_types(name, color)')
  ...
  const handleToggleCancel = async (s: Schedule) => {
    ...
    try {
      const { error: err } = await supabase
        .from('gym_class_schedules')
        .update({ is_cancelled: !s.is_cancelled })
        .eq('id', s.id)
      if (err) throw err
  ...
  const handleMoveAndCancel = async () => {
    ...
    try {
      ...
      const { error: cErr } = await supabase.from('gym_class_schedules').update({
        is_cancelled: true,
  ...
  const handleDelete = async (s: Schedule) => {
    await supabase.from('gym_class_schedules').delete().eq('id', s.id)
  ...
  const handleBulkSubmit = async (e: React.FormEvent) => {
    ...
    const { error: err } = await supabase.from('gym_class_schedules').insert(rows)
    setSaving(false)
    if (err) { setFormError(err.message); return }
  ...
  return (
    <div>
      <div className="page-header">
        <h2 className="page-title">Kelola Jadwal</h2>
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th>Quota</th><th>Booked</th><th>Sisa</th><th>Status</th><th>Aksi</th>
            </tr>
          </thead>
          <tbody>
            {(() => {
              ...
              const displayData = search
                ...
                : data
              if (loading) return <tr className="loading-row"><td colSpan={10}>Memuat...</td></tr>
              if (displayData.length === 0) return <tr><td colSpan={10} className="empty-state">{search ? 'Tidak ada hasil' : 'Tidak ada jadwal'}</td></tr>
            ...
            <form onSubmit={handleSave}>
              <div className="form-group">
                ...
                <select value={form.class_type_id} onChange={e => {
                  ...
                  {classTypes.map(ct => <option key={ct.id} value={ct.id}>{ct.name} ({ct.duration_minutes} mnt)</option>)}
                </select>
              </div>
              <div className="form-group">
      ...
      {/* Cancel confirmation + guard booking confirmed */}
      {confirmCancel && (() => {
            ...
            {hasConfirmed && (
              <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: '12px 14px', marginBottom: 16 }}>
                <p style={{ margin: 0, fontSize: 13, color: '#991B1B', lineHeight: 1.6 }}>
                  ...
                  tetap ada tapi hilang dari tampilan, dan uang yang sudah masuk tidak otomatis di-refund.
                  Pindahkan peserta ke jadwal lain dulu, atau lanjutkan hanya jika benar-benar yakin.
                </p>
              </div>
            )}
