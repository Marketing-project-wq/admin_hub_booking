...
interface ClassType {
  id: string; name: string; description: string | null; price_guest: number;
  duration_minutes: number; quota: number; color: string; is_active: boolean
}

const emptyForm = (): Partial<ClassType> => ({
  name: '', description: '', price_guest: 0, duration_minutes: 60, quota: 10,
  color: '#C0392B', is_active: true,
})

export default function GymClassTypes() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: rows, error: err } = await supabase
      ...
      .select('id, name, description, price_guest, duration_minutes, quota, color, is_active')
      .order('name')
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      price_guest: Number(form.price_guest) || 0,
    ...
    const { error: err } = editId
      ? await supabase.from('gym_class_types').update(payload).eq('id', editId)
      : await supabase.from('gym_class_types').insert(payload)
    setSaving(false)
  ...
  const toggleActive = async (ct: ClassType) => {
    await supabase.from('gym_class_types').update({ is_active: !ct.is_active }).eq('id', ct.id)
    fetchData()
  }
    ...
    <div>
      ...
      <div className="table-wrap">
        <table className="data-table">
          <thead>
            <tr>
              <th>Name</th><th>Durasi</th><th>Harga Guest</th><th>Kuota</th>
            ...
            {loading ? <tr className="loading-row"><td colSpan={7}>Memuat...</td></tr>
              ...
              : data.map(ct => (
                <tr key={ct.id}>
                  ...
                  <td>{ct.duration_minutes} mnt</td>
                  <td>{fmtRp(ct.price_guest)}</td>
                  <td style={{ textAlign: 'center' }}>{ct.quota}</td>
                  <td>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <input type="number" min={1} value={f.duration_minutes ?? 60} onChange={e => setForm(p => ({ ...p, duration_minutes: Number(e.target.value) }))} />
                </div>
              </div>
              <div className="form-row">
                <div className="form-group">
                  <label>Kuota</label>
                  <input type="number" min={1} value={f.quota ?? 10} onChange={e => setForm(p => ({ ...p, quota: Number(e.target.value) }))} />
                </div>
                <div className="form-group">
                  <label>Color (#hex)</label>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <input type="color" value={f.color || '#C0392B'} onChange={e => setForm(p => ({ ...p, color: e.target.value }))} style={{ width: 48, height: 38, padding: 2, cursor: 'pointer' }} />
                    <input type="text" value={f.color || '#C0392B'} onChange={e => setForm(p => ({ ...p, color: e.target.value }))} style={{ flex: 1 }} placeholder="#C0392B" />
                  </div>
                </div>
              </div>
