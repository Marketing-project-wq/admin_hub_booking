...
interface Coach {
  id: string; name: string; photo_url: string | null; speciality: string | null;
  bio: string | null; sort_order: number | null; is_active: boolean
}

const emptyForm = (): Partial<Coach> => ({ name: '', photo_url: '', speciality: '', bio: '', sort_order: null, is_active: true })
...
export default function GymCoaches() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: rows, error: err } = await supabase
      .from('gym_coaches')
      .select('id, name, photo_url, speciality, bio, sort_order, is_active')
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const { error: err } = editId
      ? await supabase.from('gym_coaches').update(payload).eq('id', editId)
      : await supabase.from('gym_coaches').insert(payload)
    setSaving(false)
  ...
  const toggleActive = async (c: Coach) => {
    await supabase.from('gym_coaches').update({ is_active: !c.is_active }).eq('id', c.id)
    fetchData()
  }
    ...
    <div>
      ...
      <div className="table-wrap">
        <table className="data-table">
          <thead><tr><th>Nama</th><th>Spesialisasi</th><th>Urutan</th><th>Status</th><th>Aksi</th></tr></thead>
          <tbody>
            {loading ? <tr className="loading-row"><td colSpan={5}>Memuat...</td></tr>
              : data.length === 0 ? <tr><td colSpan={5} className="empty-state">Tidak ada coach</td></tr>
              : data.map(c => (
                <tr key={c.id}>
                  <td style={{ fontWeight: 600 }}>{c.name}</td>
                  <td>{c.speciality || '-'}</td>
                  <td style={{ textAlign: 'center' }}>{c.sort_order ?? '-'}</td>
          ...
          <div className="modal-box">
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="form-row">
                ...
                <div className="form-group"><label>Urutan (sort order)</label><input type="number" value={f.sort_order ?? ''} onChange={e => setForm(p => ({ ...p, sort_order: e.target.value === '' ? null : Number(e.target.value) }))} /></div>
              </div>
